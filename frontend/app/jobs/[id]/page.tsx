    ...
    <div className="bg-ivory min-h-screen">
      ...
      <div className="bg-navy border-b border-navy-border">
        ...
        <div className="max-w-screen-xl mx-auto px-6 lg:px-10 py-8">
          <Link href="/jobs"
            className="text-[10px] font-sans font-semibold uppercase tracking-[0.2em] text-[#8A9DB5] hover:text-brass transition-colors">
            ← Opportunities
          </Link>

          <div className="mt-5 lg:max-w-[70%]">
            <span className="text-[10px] font-sans font-semibold uppercase tracking-[0.2em] text-brass block mb-3">
              {job.category}
            </span>
            <h1 className="font-serif text-2xl sm:text-3xl font-bold text-surface leading-tight">
              {job.title}
            </h1>
            <p className="mt-2 text-[15px] text-[#8A9DB5]">{job.company}</p>
            <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-[13px] text-[#8A9DB5]">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {job.skills.map((s) => (
                <span key={s}
                  className="text-[11px] px-2.5 py-0.5 font-sans text-[#8A9DB5] border border-navy-border">
                  {s}
                </span>
              ))}
            </div>
            ...
            <div className="bg-surface border border-border divide-y divide-border">
              {job.description && (
                <Sect title="About the Role">
                  <p className="text-[14px] text-muted leading-relaxed">{job.description}</p>
                </Sect>
              )}
              ...
              {job.education && (
                <Sect title="Education">
                  <p className="text-[14px] text-muted">{job.education}</p>
                </Sect>
              )}

              {job.aboutCompany && (
                <Sect title={`About ${job.company}`}>
                  <p className="text-[14px] text-muted leading-relaxed">{job.aboutCompany}</p>
                </Sect>
              )}
            </div>
...
function Sect({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="px-8 py-7">
      <p className="text-[10px] font-sans font-semibold uppercase tracking-[0.2em] text-brass mb-4">
        {title}
      </p>
      {children}
    </div>
